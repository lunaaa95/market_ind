...
def code_2_embedding_f():
    with open('data/embeddings.npy', 'r') as f:
        embeddings = np.array([i.strip().split(" ") for i in f.readlines()][1:])

    code_names = embeddings[:,0].astype(np.unicode) # 4724,
    code_embeddings = [i for i in np.array(embeddings[:,1:]).astype(np.float32)] # 4724,256
    code_2_embedding = dict(zip(code_names, code_embeddings))
    np.save('./data/code_2_embedding.npy', code_2_embedding)
